...
describe("Paywall", () => {
  ...
  it("states the purchase is one-time — the portfolio never sells subscriptions", async () => {
    ...
    // The lede is one flowing paragraph — the promise line runs straight into
    // the headline inside a single nested <Text>, so the rendered node's full
    // text is the concatenation of both. `exact: false` matches the headline
    // as a substring of that paragraph rather than requiring it be the whole
    // node's content.
    expect(getByText(t("antiSubHeadline"), { exact: false })).toBeTruthy();
  });
